...
-- TODO: restrict camera to map boundaries
    -- local target_x = mid(0, p_x - viewport_size / 2, map_width - viewport_size / 2)
    -- local target_y = mid(0, p_y - viewport_size / 2, map_height - viewport_size / 2)
...
function extract_accel(p_x, p_y)
  ...
  if (mb == 1) then
    ...
    local const m_angle = atan2(mx - viewport_px, my - viewport_py)
...
function _update()
  ...
  for i = 1, #world.players do
    ...
    local v_accel = (p == p_selected) and v_accel or v_zero

    players.update_velocity(p, pm, v_accel)
